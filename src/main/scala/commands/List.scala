...
trait ListCommands {
  ...
  case class lrange[A](key: Any, start: Int = 0, end: Int = -1)(implicit format: Format, parse: Parse[A]) extends Command(MultiBulkAsFlat[A])
  ...
  case class lindex[A](key: Any, index: Int)(implicit format: Format, parse: Parse[A]) extends Command(Bulk[A])
  ...
  case class lpop[A](key: Any)(implicit format: Format, parse: Parse[A]) extends Command(Bulk[A])

  // RPOP
  // atomically return and remove the first (LPOP) or last (RPOP) element of the list
  case class rpop[A](key: Any)(implicit format: Format, parse: Parse[A]) extends Command(Bulk[A])

  // RPOPLPUSH
  // Remove the first count occurrences of the value element from the list.
  case class rpoplpush[A](srcKey: Any, dstKey: Any)(implicit format: Format, parse: Parse[A]) extends Command(Bulk[A])

  case class blpop[A](key: Any, values: Iterable[Any], timeoutInSeconds: Int)(implicit format: Format, parse: Parse[A]) extends Command(MultiBulk[A]) {
    override def args = arg1(key) ++ values.iterator ++ arg1(timeoutInSeconds)
  }

  case class brpop[A](key: Any, values: Iterable[Any], timeoutInSeconds: Int)(implicit format: Format, parse: Parse[A]) extends Command(MultiBulk[A]) {
    override def args = arg1(key) ++ values.iterator ++ arg1(timeoutInSeconds)
  }
}
