...
object RedisSet {
  def apply[A](name: String)(implicit conn: AkkaRedisClient, format: Format, parser: Parse[A]): RedisSet[A] =
    new RedisSet[A](name)(conn, format, parser)
}

class RedisSet[A](name: String)(implicit conn: AkkaRedisClient, format: Format, parser: Parse[A]) extends mutable.Set[A] {
