...
private[redis] trait ParseResult {
  implicit def parseBulk(result: Option[ByteString]) = new ParseBulk[({ type λ[α] = α })#λ](result)(implicitly, NoTimeout)
  implicit def parseBulk(result: Future[Option[ByteString]])(implicit timeout: Timeout) = new ParseBulk[Future](result)
  implicit def parseBulk(result: Queued[Future[Option[ByteString]]])(implicit timeout: Timeout) = new ParseBulk[({ type λ[α] = Queued[Future[α]] })#λ](result)

  implicit def parseMultiBulk(result: Option[List[Option[ByteString]]]) = new ParseMultiBulk[({ type λ[α] = α })#λ](result)(implicitly, NoTimeout)
  implicit def parseMultiBulk(result: Future[Option[List[Option[ByteString]]]])(implicit timeout: Timeout) = new ParseMultiBulk[Future](result)
  implicit def parseMultiBulk(result: Queued[Future[Option[List[Option[ByteString]]]]])(implicit timeout: Timeout) = new ParseMultiBulk[({ type λ[α] = Queued[Future[α]] })#λ](result)

  implicit def parseMultiBulkList(result: List[Option[ByteString]]) = new ParseMultiBulkList[({ type λ[α] = α })#λ](result)(implicitly, NoTimeout)
  implicit def parseMultiBulkList(result: Future[List[Option[ByteString]]])(implicit timeout: Timeout) = new ParseMultiBulkList[Future](result)
  implicit def parseMultiBulkList(result: Queued[Future[List[Option[ByteString]]]])(implicit timeout: Timeout) = new ParseMultiBulkList[({ type λ[α] = Queued[Future[α]] })#λ](result)

  implicit def parseMultiBulkFlat(result: Option[List[ByteString]]) = new ParseMultiBulkFlat[({ type λ[α] = α })#λ](result)(implicitly, NoTimeout)
  implicit def parseMultiBulkFlat(result: Future[Option[List[ByteString]]])(implicit timeout: Timeout) = new ParseMultiBulkFlat[Future](result)
  implicit def parseMultiBulkFlat(result: Queued[Future[Option[List[ByteString]]]])(implicit timeout: Timeout) = new ParseMultiBulkFlat[({ type λ[α] = Queued[Future[α]] })#λ](result)

  implicit def parseMultiBulkFlatList(result: List[ByteString]) = new ParseMultiBulkFlatList[({ type λ[α] = α })#λ](result)(implicitly, NoTimeout)
  implicit def parseMultiBulkFlatList(result: Future[List[ByteString]])(implicit timeout: Timeout) = new ParseMultiBulkFlatList[Future](result)
  implicit def parseMultiBulkFlatList(result: Queued[Future[List[ByteString]]])(implicit timeout: Timeout) = new ParseMultiBulkFlatList[({ type λ[α] = Queued[Future[α]] })#λ](result)

  implicit def parseMultiBulkSet(result: Set[ByteString]) = new ParseMultiBulkSet[({ type λ[α] = α })#λ](result)(implicitly, NoTimeout)
  implicit def parseMultiBulkSet(result: Future[Set[ByteString]])(implicit timeout: Timeout) = new ParseMultiBulkSet[Future](result)
  implicit def parseMultiBulkSet(result: Queued[Future[Set[ByteString]]])(implicit timeout: Timeout) = new ParseMultiBulkSet[({ type λ[α] = Queued[Future[α]] })#λ](result)

  implicit def parseMultiBulkMap(result: Map[ByteString, ByteString]) = new ParseMultiBulkMap[({ type λ[α] = α })#λ](result)(implicitly, NoTimeout)
  implicit def parseMultiBulkMap(result: Future[Map[ByteString, ByteString]])(implicit timeout: Timeout) = new ParseMultiBulkMap[Future](result)
  implicit def parseMultiBulkMap(result: Queued[Future[Map[ByteString, ByteString]]])(implicit timeout: Timeout) = new ParseMultiBulkMap[({ type λ[α] = Queued[Future[α]] })#λ](result)

  implicit def parseMultiBulkScored(result: List[(ByteString, Double)]) = new ParseMultiBulkScored[({ type λ[α] = α })#λ](result)(implicitly, NoTimeout)
  implicit def parseMultiBulkScored(result: Future[List[(ByteString, Double)]])(implicit timeout: Timeout) = new ParseMultiBulkScored[Future](result)
  implicit def parseMultiBulkScored(result: Queued[Future[List[(ByteString, Double)]]])(implicit timeout: Timeout) = new ParseMultiBulkScored[({ type λ[α] = Queued[Future[α]] })#λ](result)

  private[redis] class ParseBulk[Result[_]](value: Result[Option[ByteString]])(implicit f: ResultFunctor[Result], timeout: Timeout) {
    def parse[A: Parse]: Result[Option[A]] = f.fmap(value)(_.map(Parse(_)))
  }
  private[redis] class ParseMultiBulk[Result[_]](value: Result[Option[List[Option[ByteString]]]])(implicit f: ResultFunctor[Result], timeout: Timeout) {
    def parse[A: Parse]: Result[Option[List[Option[A]]]] = f.fmap(value)(_.map(_.map(_.map(Parse(_)))))
  }
  private[redis] class ParseMultiBulkList[Result[_]](value: Result[List[Option[ByteString]]])(implicit f: ResultFunctor[Result], timeout: Timeout) {
  ...
  private[redis] class ParseMultiBulkFlat[Result[_]](value: Result[Option[List[ByteString]]])(implicit f: ResultFunctor[Result], timeout: Timeout) {
    def parse[A: Parse]: Result[Option[List[A]]] = f.fmap(value)(_.map(_.map(Parse(_))))
  }
  private[redis] class ParseMultiBulkFlatList[Result[_]](value: Result[List[ByteString]])(implicit f: ResultFunctor[Result], timeout: Timeout) {
    def parse[A: Parse]: Result[List[A]] = f.fmap(value)(_.map(Parse(_)))
  }
  private[redis] class ParseMultiBulkSet[Result[_]](value: Result[Set[ByteString]])(implicit f: ResultFunctor[Result], timeout: Timeout) {
    def parse[A: Parse]: Result[Set[A]] = f.fmap(value)(_.map(Parse(_)))
  }
  private[redis] class ParseMultiBulkMap[Result[_]](value: Result[Map[ByteString, ByteString]])(implicit f: ResultFunctor[Result], timeout: Timeout) {
    def parse[K: Parse, V: Parse]: Result[Map[K, V]] = f.fmap(value)(_.map(kv ⇒ (Parse[K](kv._1), Parse[V](kv._2))))
  }
  private[redis] class ParseMultiBulkScored[Result[_]](value: Result[List[(ByteString, Double)]])(implicit f: ResultFunctor[Result], timeout: Timeout) {
