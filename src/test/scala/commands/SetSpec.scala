...
class SetOperationsSpec extends Spec 
  ...
  describe("sinter") {
    it("should return intersection") {
      ...
      r send sinter(Seq("set-1", "set-2")) map (_.toSet) should equal(Some(Set("foo", "baz")))
      r send sinter(Seq("set-1", "set-3")) map (_.toSet) should equal(Some(Set.empty))
    }
    it("should return empty set for non-existing key") {
      r send sadd("set-1", "foo") should equal(true)
      r send sadd("set-1", "bar") should equal(true)
      r send sadd("set-1", "baz") should equal(true)
      r send sinter(Seq("set-1", "set-4")) map (_.toSet) should equal(Some(Set())) 
    }
  }
  ...
  describe("sunion") {
    it("should return union") {
      ...
      r send sunion(Seq("set-1", "set-2")) map (_.toSet) should equal(Some(Set("foo", "bar", "baz", "bat")))
      r send sunion(Seq("set-1", "set-3")) map (_.toSet) should equal(Some(Set("foo", "bar", "baz", "for", "bat", "bay")))
    }
    it("should return empty set for non-existing key") {
      r send sadd("set-1", "foo") should equal(true)
      r send sadd("set-1", "bar") should equal(true)
      r send sadd("set-1", "baz") should equal(true)
      r send sunion(Seq("set-1", "set-2")) map (_.toSet) should equal(Some(Set("foo", "bar", "baz")))
    }
  }
  ...
  describe("sdiff") {
    it("should return diff") {
      ...
      r send sdiff(Seq("set-1", "set-2", "set-3")) map (_.toSet) should equal(Some(Set("bar")))
    }
    it("should treat non-existing keys as empty sets") {
      r send sadd("set-1", "foo") should equal(true)
      r send sadd("set-1", "bar") should equal(true)
      r send sadd("set-1", "baz") should equal(true)
      r send sdiff(Seq("set-1", "set-2")) map (_.toSet) should equal(Some(Set("foo", "bar", "baz")))
    }
  }
  ...
  describe("smembers") {
    it("should return members of a set") {
      ...
      r send sadd("set-1", "bar") should equal(true)
      r send sadd("set-1", "baz") should equal(true)
      r send smembers("set-1") map (_.toSet) should equal(Some(Set("foo", "bar", "baz")))
    }
    it("should return None for an empty set") {
      r send smembers("set-1") map (_.toSet) should equal(Some(Set()))
    }
  }
