...
class OperationsSpec extends Spec
  ...
  describe("sort") {
    it("should do a simple sort") {
      ...
      (r send sort("sortlist")).map(_.flatMap(_.map(_.toInt))) should be(Some(List(1, 1, 3, 4, 5, 6, 9, 47)))
    }
    it("should do a lexical sort") {
      List("lorem", "ipsum", "dolor", "sit", "amet", 3, 7) foreach (r send lpush("sortlist", _))
      r send sort[String]("sortlist", alpha = true) map (_.flatten) should be(Some(List("3", "7", "amet", "dolor", "ipsum", "lorem", "sit")))
    }
